...
- **Expo SDK 54 (downgraded from 57, 2026-07)**: Expo Go runs exactly one SDK, and the store serves older iOS devices the SDK 54 client — the newest SDK the owner's phone can actually run. The spec floor is SDK 51+, so 54 complies; upgrade again when the device situation changes.
